...
def subscribe_to_sns(sns_topic_name):
    ...
    lambda_complete_arn = f'arn:aws:lambda:{region}:{account_id}:function:{this_lambda_name}'
    
    subscriptionArn = sns_client.subscribe(TopicArn=sns_complete_arn,Protocol="lambda", Endpoint=lambda_complete_arn, ReturnSubscriptionArn=True)['SubscriptionArn'];
    print(f"Lambda :: {this_lambda_name} is now subscribed to SNS :: {sns_complete_arn} and subscriptionArn is {subscriptionArn}")
...
def setup_agent_after_create(bedrock_agent_client, 
                             agent_id,
                             agent_alias_name,
                             lambda_function,
                             bucket_name,
