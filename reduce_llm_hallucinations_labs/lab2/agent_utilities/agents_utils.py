...
def format_final_response(question_id, question, final_answer, lab_number, turn_number, show_detailed=True):
    ...
    if show_detailed is True:
        ...
        response_df = pd.DataFrame( list(zip(question_id_list, question_list, finalAPIResponse, kbResponse, hallucinationScore)), 
                                      columns=["Question ID", "User Question", "Agent/Chatbot Response", "KB Response", "Answer Score"],)
        response_df.style.set_properties(**{'text-align': 'left', 'border': '1px solid black'})
        with pd.option_context("display.max_colwidth", None):
            pretty_print(response_df)
    else:
        # Store and print as a dataframe
        response_df = pd.DataFrame(list(zip(question_id_list, question_list,  agent_answer_list )), 
